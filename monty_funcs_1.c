...
void monty_push(stack_t **stack, unsigned int line_number)
{
	...
	if (new == NULL)
	{
		set_opp_tok_error(malloc_error());
		return;
	}

	if (op_toks[1] == NULL)
	{
		set_opp_tok_error(no_int_error(line_number));
		return;
	}
	...
	if (check_mode(*stack) == STACK) /* STACK mode insert infront */
	{
		tmp = (*stack)->next;
		new->prev = *stack;
		new->next = tmp;
...
void monty_pint(stack_t **stack, unsigned int line_number)
{
	if ((*stack)->next == NULL)
	{
		set_opp_tok_error(pint_error(line_number));
		return;
	}
...
void monty_pop(stack_t **stack, unsigned int line_number)
{
	...
	if ((*stack)->next == NULL)
	{
		set_opp_tok_error(pop_error(line_number));
		return;
	}
...
void monty_swap(stack_t **stack, unsigned int line_number)
{
	...
	{
		set_opp_tok_error(shorty_stack_error(line_number, "swap"));
		return;
	}
